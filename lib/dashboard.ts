...
export async function getCurrentDoctor(): Promise<Doctor> {
  ...
  const { data, error } = await supabase.from('doctors').select('id,auth_user_id,doctor_name,clinic_name,specialization,slug,gmb_review_link,city,phone,logo_url,plan,theme_config,knowledge_base').eq('auth_user_id', user.id).maybeSingle();
