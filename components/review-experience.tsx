...
export function ReviewExperience({ doctor, experienceKeywords, topServices, scanId, isStarter: _isStarter, isGrowth: _isGrowth }: { doctor: Doctor; experienceKeywords: string[]; topServices: string[]; scanId: string | null; isStarter: boolean; isGrowth: boolean }) {
  ...
  const initials = doctor.doctor_name.replace(/^dr\.?\s*/i, "").split(/\s+/).map((part) => part[0]).slice(0, 2).join("").toUpperCase();
  ...
  const BrandHeader = () => <a href="https://medirank.vyaparwallah.com" target="_blank" rel="noreferrer" className="relative z-50 mx-auto flex min-h-14 w-full max-w-xl flex-nowrap items-center justify-center gap-1 whitespace-nowrap rounded-2xl border border-slate-200 bg-white px-3 text-sm font-black shadow-sm sm:text-base"><span className="text-[#0A4C95]">MediRank</span><span className="text-slate-700">by</span><span className="text-[#0A4C95]">Vyapar</span><span className="text-[#F37021]">Wallah</span><ExternalLink size={14} className="ml-1 text-slate-500" /></a>;
  ...
  return <main style={style} className="min-h-[100dvh] bg-[var(--patient-bg)] pb-14 text-slate-950">
    ...
    <div className="mx-auto w-full max-w-xl space-y-20 px-3 pt-4 sm:px-5">
      <header className="relative z-30 rounded-3xl bg-white px-4 py-6 text-center shadow-xl">{doctor.logo_url ? <img src={doctor.logo_url} alt={doctor.clinic_name} className="mx-auto h-16 w-16 rounded-2xl object-contain ring-1 ring-slate-200" /> : <span className="mx-auto grid h-16 w-16 place-items-center rounded-2xl bg-[#0A4C95] text-xl font-black text-white">{initials}</span>}<p className="mt-3 text-sm font-black text-[#0A4C95]">{doctor.clinic_name}</p><h1 className="mt-2 text-2xl font-black">{t.welcome}<br />{doctor.doctor_name}?</h1><button type="button" onClick={() => setCurrentLanguage(null)} className="mt-4 min-h-12 px-3 text-sm font-bold text-[#0A4C95]">🌐 {currentLanguage === "english" ? "English" : "Hinglish"}</button></header>
