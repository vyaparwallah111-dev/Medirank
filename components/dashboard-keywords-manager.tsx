...
export function DashboardKeywordsManager({ initialItems, businessType = 'doctor' }: { initialItems: KeywordItem[]; businessType?: 'doctor' | 'coaching' }) {
  const isCoaching = businessType === 'coaching';
  const defaultCategory = isCoaching ? 'teaching' : 'behavior';
  ...
  const categoryOptions = isCoaching
    ? [
        { value: 'teaching', label: '📚 Teaching & Concepts' },
        { value: 'faculty', label: '👨‍🏫 Faculty & Mentorship' },
        { value: 'environment', label: '🕌 Environment & Facilities' },
        { value: 'result', label: '🎯 Results & Target Exam' },
      ]
    : [
  ...
  return <div>
    <form ref={formRef} action={handleAdd} className="card mt-6 grid gap-3 p-4 sm:mt-8 sm:grid-cols-[1fr_200px_auto] sm:p-6">
      <div>
        <label className="label">Keyword / Highlight</label>
        <input
          name="keyword"
          className="input"
          placeholder={isCoaching ? "e.g. NEET Islamic environment / Doubt clearing sessions" : "e.g. Gentle treatment / Painless root canal"}
          required
          disabled={isPending}
        />
      </div>
      <div>
        ...
        <select name="category" className="input" defaultValue={defaultCategory} disabled={isPending}>
